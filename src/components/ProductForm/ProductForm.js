...
function ProductForm() {
  ...
  const image = watchFields.image
    ? watchFields.image
    : location.state?.product.image;
  const activeCategory = categories.find(
    (category) => category.name === watchFields.category
  );
  ...
  const onSubmit = (data, e) => {
    ...
    formData.append(
      "category",
      data["newCategory"]
        ? data["newCategory"].toLowerCase()
        : data["category"].toLowerCase()
    );
    formData.append(
      "subCategory",
      data["newSubCategory"]
        ? data["newSubCategory"].toLowerCase()
        : data["subCategory"].toLowerCase()
    );
    formData.append(
      "brand",
      data["newBrand"]
        ? data["newBrand"].toLowerCase()
          ...
          <div className={styles.formFooter}>
            ...
            {succesRequest && (
              <p className={styles.succes}>
                Товар успешно {location.state ? "изменен" : "добавлен"}!
              </p>
            )}
